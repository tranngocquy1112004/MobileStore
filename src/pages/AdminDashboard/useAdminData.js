...
import { LOCAL_STORAGE_ORDERS_KEY, LOCAL_STORAGE_USERS_KEY, MESSAGES } from "./constants";
...
export const useAdminData = () => {
  ...
  useEffect(() => {
    const loadData = () => {
      ...
      const storedOrders = sortOrdersByDateDesc(
        readJsonFromStorage(LOCAL_STORAGE_ORDERS_KEY, [])
      );
  ...
  const handleDeleteUser = (usernameToDelete) => {
    if (
      !window.confirm(
        `Bạn có chắc muốn xóa người dùng "${usernameToDelete}" và tất cả đơn hàng của họ?`
      )
    ) {
      return;
    }
