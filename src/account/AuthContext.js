import React, { createContext, useState, useEffect, useCallback } from "react"; // Import các hook cần thiết từ thư viện React: createContext để tạo Context (một cách để chia sẻ dữ liệu giữa các component mà không cần truyền props thủ công qua mọi cấp), useState để quản lý trạng thái cục bộ (trạng thái đăng nhập và thông tin người dùng), useEffect để thực hiện các tác vụ phụ (side effects) như đọc dữ liệu từ localStorage khi component được mount, và useCallback để ghi nhớ (memoize) các hàm xử lý sự kiện (login, logout), giúp tối ưu hiệu suất bằng cách ngăn các hàm này được tạo lại không cần thiết qua mỗi lần render.
