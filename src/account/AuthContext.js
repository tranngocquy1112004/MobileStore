...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      try {
        ...
        // Thêm kiểm tra sâu hơn cho parsedUser để đảm bảo nó có cấu trúc mong đợi (ví dụ: có username)
        if (parsedUser && typeof parsedUser === 'object' && parsedUser !== null && parsedUser.username) {
            ...
            // Trường hợp chuỗi trong localStorage không phải là null/undefined nhưng parse ra lại là null/undefined,
            // hoặc dữ liệu parse được không có cấu trúc mong đợi (thiếu username).
            console.warn("Dữ liệu currentUser trong localStorage không hợp lệ hoặc thiếu thông tin cần thiết, đã xóa."); // Ghi cảnh báo ra console
  ...
  const login = useCallback((userData) => {
    // Kiểm tra dữ liệu người dùng đầu vào có hợp lệ không
    if (!userData || typeof userData !== 'object' || userData === null || !userData.username) {
        console.error("Dữ liệu người dùng không hợp lệ khi gọi login:", userData);
    ...
    try {
        ...
        // alert("Lỗi khi lưu thông tin đăng nhập.");
    }
  ...
  const logout = useCallback(() => {
    ...
    try {
        ...
        // Thông báo lỗi cho người dùng nếu không xóa được
